...
for i in range(2):
    ax[i].set_xlabel('x(m)')
    ax[i].set_ylabel('y(m)')
    ax[i].set_title('Original Point Cloud')
    ax[i].plot(ground_truth(1), ground_truth(2), '*g', ms=10)
    ax[i].plot(radar_pos(1), radar_pos(2), 'dc', ms=10)
    ax[i].set_xlim([-5, 5])
    ax[i].set_ylim([0, 10])
    ...
    ax[i].plot([radar_pos(1), (top_wall_y - fov_line_z) / fov_line_k], [radar_pos(2), top_wall_y])
    line.append(ax[i].plot())
        ...
        if magicOK:
            ...
            for tlvIdx in range(numTLVs):
                ...
                if tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS:
                    ...
                    # 过滤并映射
                    pointCloudNLOS = nlosFilterAndMapping(pointCloud)
...
def main(args):
    ...
    try:
        ani = animation.FuncAnimation(fig, visualize, readAndParseData14xx)
